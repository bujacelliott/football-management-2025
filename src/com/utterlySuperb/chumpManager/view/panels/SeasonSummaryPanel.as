package com.utterlySuperb.chumpManager.view.panels
{
   ...
   public class SeasonSummaryPanel extends Panel
   {
      ...
      public function SeasonSummaryPanel()
      {
         ...
         if(_loc2_.leagues[0].entrants.length - _loc3_ > 2)
         {
            if(_loc3_ < 5)
            {
               _loc5_ += "<br><br>" + CopyManager.getCopy("eligleForEuro").replace("{cupName}",CopyManager.getCopy("europeanCup"));
            }
            _loc6_ = _loc2_.leagues[0].getCompetitionInfo(_loc2_.playerClub);
            _loc7_ = (_loc2_.leagues[0].entrants.length - _loc6_.currentPosition) * 10000;
         ...
         {
            _loc5_ += "<br><br>But because you got relegated it\'s all over for you. Try agains, maybe with a better team!";
         }
